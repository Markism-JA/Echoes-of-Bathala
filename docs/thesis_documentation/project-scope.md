...
This document defines the three-tiered criteria for project completion, allowing for adjustments based on time, resources, and technical constraints. The project's success will be measured against one of these three defined tiers. We can only define which of these 3 after the implementation phase based on the time and other resource available to us during that time. This document is constructed directly against the [Scope](thesis_documentation/01_Introduction/06-SCOPE_AND_LIMITATION_OF_THE_STUDY.md), [SOP](thesis_documentation/01_Introduction/03-STATEMENT-OF-THE-PROBLEM.md) and [Objective](thesis_documentation/01_Introduction/05-OBJECTIVES_OF_THE_STUDY.md).
