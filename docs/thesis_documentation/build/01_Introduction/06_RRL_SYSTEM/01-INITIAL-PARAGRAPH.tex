...
Virtual economies are vulnerable to ``Sybil attacks,'' where automated scripts create thousands of accounts to drain faucets \autocite{douceur2002sybil}. However, applying harsh penalties indiscriminately would harm legitimate players. To balance this, the system implements a \textbf{Daily Wallet Allowance} \( A_{\text{daily}} \).
...
The safeguards described above feed into the On-Chain Treasury, which acts as the ``Market Maker of Last Resort'' \autocite{gonzalez2025stablecoin}. The treasury is funded by accumulated Processing Fees \( \tau \) and a portion of penalties from the Diminishing Returns system.
